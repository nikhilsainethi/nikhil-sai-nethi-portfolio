...
export function HeroAnimation() {
  ...
  return (
    <div
      ref={containerRef}
      className="absolute inset-0 -z-10 overflow-hidden bg-[var(--background)]"
    >
      {/* Dynamic Grid Background */}
      <div className="absolute inset-0 bg-[linear-gradient(var(--grid-line)_1px,transparent_1px),linear-gradient(90deg,var(--grid-line)_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_50%,#000_20%,transparent_100%)]" />

      {/* Massive glowing orbs driven by mouse parallax */}
      <motion.div
        style={{ x: useTransform(mouseX, x => x * -2), y: useTransform(mouseY, y => y * -2) }}
        className="absolute -top-32 -left-32 h-[40vw] w-[40vw] rounded-full bg-gradient-to-br from-accent/20 to-transparent blur-[120px]"
      />
      <motion.div
        style={{ x: useTransform(mouseX, x => x * 1.5), y: useTransform(mouseY, y => y * 1.5) }}
        className="absolute top-1/2 -right-32 h-[35vw] w-[35vw] -translate-y-1/2 rounded-full bg-gradient-to-bl from-[var(--warm-accent)]/18 to-transparent blur-[140px]"
      />
      <motion.div
        style={{ x: useTransform(mouseX, x => x * 0.8), y: useTransform(mouseY, y => y * 0.8) }}
        className="absolute -bottom-32 left-1/3 h-[45vw] w-[45vw] rounded-full bg-gradient-to-tr from-accent/14 to-transparent blur-[150px]"
      />
      ...
      >
        <span className="text-[18vw] font-black leading-none whitespace-nowrap tracking-tighter text-[var(--foreground)]">ENGINEER</span>
      </motion.div>
      <motion.div
      ...
      >
        <span className="text-[18vw] font-black leading-none whitespace-nowrap tracking-tighter text-[var(--foreground)]">OBSERVABILITY</span>
      </motion.div>
    </div>
  );
}
